...
# this is the email configuration file 
EMAIL_CONFIG_FILE="/etc/user_emails.conf"
if [[ -f "$EMAIL_CONFIG_FILE" ]]; then
    source "$EMAIL_CONFIG_FILE"
fi

# loads the email configuration file because it would be used later 
...
# loads sysadmin name so that in the rpeort it will show 
SYSADMIN_NAME_FILE="/etc/sysadmin_name.conf"
if [[ -f "$SYSADMIN_NAME_FILE" ]]; then
    source "$SYSADMIN_NAME_FILE"  
...
# loads secret key for python server ( it will be automaticallly updated from setup.sh) 
...
# it contains file for cybersecurity professional both name and email.
CYBERSEC_FILE="/etc/cybersecurity_professionals.conf"

#this is for the server.py when user say yes it means opt in and if users say no its opt out 
OPT_IN_FILE="/etc/dormant_opt_in.conf"
OPT_OUT_FILE="/etc/dormant_opt_out.conf"  
touch "$OPT_IN_FILE"
touch "$OPT_OUT_FILE"
...
# Users that are reactivated in last 24 hours need to skip dormant detection. 
...
# generate the token with a timestamp
generate_token() {
    local user=$1
    local timestamp
 ...
 #this code is where it is used to forward email to users
send_email_to_user() {
    local user=$1
    local email=$2
...
#most of it is basically configured from UI 
...
#checks wheather the user is dormant according to /etc/dormant.conf 
detect_dormant_user() {
...
generate_report() {
    ...
    # users that are approaching dormancy
    ...
    # email notificatons that are being send 
...
# this sends the report to the cybersecuroty professionals 
