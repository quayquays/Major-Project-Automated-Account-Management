...
send_email_to_user() {
    ...
    local deny_url="${server_url}/deactivate/${user}?token=${token}&response=no"
...
generate_report() {
    ...
    {
    echo "Dormant User Account Report 📜"
    echo "Generated on: $human_readable_time"
    echo "Report generated by: $SYSADMIN_NAME"
    echo

    echo "1) Users Deactivated via Email Link 🚫"
    echo "--------------------------------------"
    if [[ -s "$EMAIL_DEACTIVATION_LOG" ]]; then
        grep "^$today_date" "$EMAIL_DEACTIVATION_LOG" | cut -d ' ' -f1,2,3,5- | sort -u | tac | head -n 20 || echo "No users deactivated via email today."
    else
        echo "No users deactivated via email."
    fi
    echo

    echo "2) Users Manually Deactivated 🛠️"
    echo "-------------------------------"
    if [[ -s "$MANUAL_DEACTIVATION_LOG" ]]; then
        grep "^$today_date" "$MANUAL_DEACTIVATION_LOG" | cut -d ' ' -f1,2,3,5- | sort -u | tac | head -n 20 || echo "No manual deactivations recorded today."
    else
        echo "No manual deactivations recorded."
    fi
    echo

    echo "3) Users Automatically Deactivated 🤖"
    echo "-------------------------------------"
    if [[ -s "$AUTO_DEACTIVATION_LOG" ]]; then
        grep "^$today_date" "$AUTO_DEACTIVATION_LOG" | cut -d ' ' -f1,2,3,5- | sort -u | tac | head -n 20 || echo "No automatic deactivations recorded today."
    else
        echo "No automatic deactivations recorded."
    fi
    echo

    echo "4) Users Reactivated ♻️"
    echo "-----------------------"
    if [[ -s "$REACTIVATION_LOG" ]]; then
        grep "^$today_date" "$REACTIVATION_LOG" | cut -d ' ' -f1,2,3,5- | sort -u | tac | head -n 20 || echo "No reactivations recorded today."
    else
        echo "No reactivations recorded."
    fi
    echo

    echo "5) Users Approaching Dormancy ⏳ (=$((DORMANT_USERACCOUNT_DURATION - 7)) days)"
    echo "--------------------------------------------------------------"
    ...
    echo "6) Email Notifications Sent 📧"
    echo "-----------------------------"
    if [ ${#dormant_email_log[@]} -eq 0 ]; then
        echo "No email notifications sent during this run."
    else
    ...
    echo
    echo "End of Report"
    echo "Generated by $SYSADMIN_NAME"
    } > "$report_file"
